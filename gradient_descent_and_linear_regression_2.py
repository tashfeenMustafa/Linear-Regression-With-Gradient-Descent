...
## hypothesis function, h(x) = C0 + C1*x + C2x**2, (theta => C)
import numpy as np
...
while epochs < 1000:
    ...
    h_x = C0 + C1 * x_train + C2 * (x_train ** 2)  # defining the hypothesis function h(x)
    ...
    C2 = C2 - learning_rate * 2 * np.sum(error * (x_train ** 2)) / n
